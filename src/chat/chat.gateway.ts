...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async newMessage(socket: Socket, data: any) {
        ...
        console.log(`[MessageSent] :\t${data.message} from ${user.name} to group ID ${data.group}`)
    }
    ...
    async joinGroup(socket: Socket, data: any) {
        ...
        const resJoin = await this.userService.joinGroup(data.user, joinGroupDto);
        ...
        console.log(`[UserJoinGroup]:\t${user.username} joined ${resJoin.updatedGroup.groupName}`)
    }
    ...
    async leaveGroup(socket: Socket, data: any) {
        ...
        const resLeave = await this.userService.leaveGroup(data.user, leaveGroupDto)
        const resMsg = await this.messageService.newMessage(newMessageDto)
        ...
        console.log(`[UserLeaveGroup]:\t${user.username} left ${resLeave.updatedGroup.groupName}`)
    }
    ...
    async changeRoom(socket: Socket, data: any) {
        if (!data.group) {
            ...
            data.groupName = (await this.groupService.getGroupByGroupId(data.group)).groupName;
        }

        socket.emit('change-focused-room-reply', data)
        console.log(`[ChangeFocusedRoom] :\t ${socket.id} to ${data.groupName}`)
    }
