...
export class MessageService {
    constructor(@InjectModel('Message') private messageModel: Model<Message>, private userService: UserService) {}
    ...
    private async getLastReadTimeOfAGroup(groupId: string, senderId: string) {
        return (await this.userService.getUserById(senderId)).groupMembership.find(element => element.group === groupId).group;
    }

    async getUnreadMessageByGroup(groupId: string, senderId: string): Promise<Message[]> {
        const messages = await this.messageModel.find({group: groupId, sender: senderId, createdTime: {$gte: await this.getLastReadTimeOfAGroup(groupId, senderId)}});
